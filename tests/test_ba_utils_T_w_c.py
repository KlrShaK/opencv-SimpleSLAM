...
class WorldMap:
    """Holds camera poses (T_wc) and 3-D points."""
...
def generate_scene(
    ...
    *,
    ...
    for T_wc_gt in poses_gt_wc:
        ...
        wmap.poses.append(T_wc_noisy)  # store T_wc
...
def reproj_rmse(wmap: WorldMap, keypoints, frames: List[int] | None = None) -> float:
    ...
    wmap.poses are assumed to be T_wc (camera-to-world) as expected by ba_utils.
    """
    sq_err = 0.0
    count = 0
    ...
    for mp in wmap.points.values():
        for f_idx, kp_idx, descriptors in mp.observations:
            ...
            # wmap.poses[f_idx] is T_wc (camera-to-world)
            T_wc = wmap.poses[f_idx]
            X_w = mp.position
            R_wc, t_wc = T_wc[:3, :3], T_wc[:3, 3]
            X_c = R_wc.T @ (X_w - t_wc)        # using T_wc
